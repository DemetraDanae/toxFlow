...
server <- function(input, output) {
  ...
  output$slider_PhCh<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("PhCh_th","Physicochemical threshold:",min=0,max=150,value=1,step=0.1)
    }
  )
  ...
  output$slider_Bio<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("Bio_th","Biological threshold:",min=0,max=150,value=1,step=0.1)
    }
  )
  ...
  output$slider_PhChplot<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("PhCh_th1","Physicochemical thresholds:",min=0,max=100,value=c(3,5),step=0.5)
    }
  )
  ...
  output$slider_Bioplot<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("Bio_th1","Biological thresholds:",min=0,max=100,value=c(3,5),step=0.5)
    }
  )
  ...
  output$slider_PhChplot_p<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("PhCh_th1_p","Physicochemical thresholds:",min=0,max=100,value=c(3,5),step=0.05)
    }
  )
  ...
  output$slider_Bioplot_p<-renderUI(
    if(input$calc=="cos"){
      ...
      sliderInput("Bio_th1_p","Biological thresholds:",min=0,max=100,value=c(3,5),step=0.05)
    }
  )
  ...
  #netplot(input$ref,nano_ID(),input$PhCh_th1[1],input$PhCh_th1[2],input$Bio_th1[1],input$Bio_th1[2],relDescr,relBio,diametr(),categ())
  selected<-nano_ID()
  range<-nrow(selected)
  id<-c()
  rel<-c()
  ...
  for (i in seq(1,range)){
    ...
    if (i==id_ref){ #color_of_nodes
      ...
      l[i,1]<-0
      l[i,2]<-0}
    else if ((relDescr[input$ref,NP]<=input$PhCh_th1[2])&(relDescr[input$ref,NP]>input$PhCh_th1[1])&(relBio[input$ref,NP]<=input$Bio_th1[2])&(relBio[input$ref,NP]>input$Bio_th1[1])) {
      links[i,4]<-"#00CDCD"
      rad<-sample(r2,1)
      theta<-sample(f,1)
      l[i,1]<-rad*cos(theta)
    ...
    else if ((relDescr[input$ref,NP]>input$PhCh_th1[2])&(relBio[input$ref,NP]>input$Bio_th1[2])){
      links[i,4]<-"#008B8B"
      rad<-sample(r1,1)
      theta<-sample(f,1)
      l[i,1]<-rad*cos(theta)
  ...
  links<-data.frame(stringsAsFactors = TRUE)
  for (i in seq(1,range)){
    links[i,1]<-id[which(nodes$nano_number==input$ref_p)]
    links[i,2]<-paste("s0", i, sep="")
    links[i,3]<-"mention"   #type?
    id_ref<-which(nodes$nano_number==input$ref_p)
    NP<-nodes[i,2]
    if (i==id_ref){ #color_of_nodes
      links[i,4]<-"#FF3030"
      l[i,1]<-0
      l[i,2]<-0}
    else if ((relDescr_p[input$ref_p,NP]<=input$PhCh_th1_p[2])&(relDescr_p[input$ref_p,NP]>input$PhCh_th1_p[1])&(relBio_p[input$ref_p,NP]<=input$Bio_th1_p[2])&(relBio_p[input$ref_p,NP]>input$Bio_th1_p[1])) {
      links[i,4]<-"#00CDCD"
      rad<-sample(r2,1)
      theta<-sample(f,1)
      l[i,1]<-rad*cos(theta)
    ...
    else if ((relDescr_p[input$ref_p,NP]>input$PhCh_th1_p[2])&(relBio_p[input$ref_p,NP]>input$Bio_th1_p[2])){
      links[i,4]<-"#008B8B"
      rad<-sample(r1,1)
      theta<-sample(f,1)
      l[i,1]<-rad*cos(theta)
